...
class Document():
    ...
    def features(self):
        ...
        word_list.append('followedNC={}'.format(self.data.split('_')[2])
...
class Corpus():
    ...
    def __init__(self, corpus_file, include_sublabels=False):
        ...
        for line in open(corpus_file,'r'):
            ...
            followedNC = split_line[5]
            
            self.documents.append(Document("{}_{}_{}".format(speaker,sentence,followedNC), label))
...
if __name__ == '__main__':
    docs = Corpus('./gold_standard_all_ncd.txt',True)
    seed(time.time())
    shuffle(docs)
